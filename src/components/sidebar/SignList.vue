      ...
      <div class="layui-layer-content pd0">
        <div class="layui-tab layui-tab-brief">
          <ul class="layui-tab-title">
            <li :class="{'layui-this': current === 2 }" @click="choose(2)">Latest</li>
            <li :class="{'layui-this': current === 1 }" @click="choose(1)">Today fastest</li>
            <li :class="{'layui-this': current === 0 }" @click="choose(0)">Total</li>
          </ul>
          <div class="layui-tab-content fly-signin-list">
            <ul class="layui-tab-item layui-show">
              <li class="sign-item" v-for="(item,index) in lists" :key="'sign' + index">
                ...
                <span class="fly-grey" v-if="current !== 0">Sign-in on {{item.created | hours}}</span>
                <span class="fly-grey" v-else>
                  Continuous
                  <i class="orange">{{item.count}}</i > days
                </span>
              </li>
            </ul>
          </div>
